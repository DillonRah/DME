static void Prototype()
{
    ...
    */
    ...
    {
        int sum = 0;
        Console.WriteLine("Enter your NHS login");
        string nhslinkfunction = Console.ReadLine();
        int value;
        int z = 10;
        if (int.TryParse(nhslinkfunction, out value))
        {
            char[] nhscheck = nhslinkfunction.ToCharArray();
            if (nhscheck.Length == 10)
            {
                for (int i = 0; nhscheck.Length - 1 > i; i++, z--)
                {
                    sum += Convert.ToInt16(nhscheck[i]) * z;
                }
                if (nhslinkfunction[9] != sum % 11) // https://www.datadictionary.nhs.uk/attributes/nhs_number.html nhs check digit algorithm
                {
                ...
                {
                    int NHSnumber = Convert.ToInt16(nhslinkfunction);
                    return NHSnumber;
                }
            }
            else
            {
                Console.WriteLine("Your NHS number must be an input of 10 numbers");
                return 0;
            }
        }
        else
        {
            // basic validation
            Console.WriteLine("You did not enter a valid NHS login");
            return 0;
        }
    }
        ...
        else
        {
            ...
            if (int.TryParse(input, out value))
            {
                if (value == 1)
                {
                    ...
                    Console.WriteLine($"You probably have {problem}");
